...
public class ShippingRateServiceTests {
	...
	// String state = "ABCDE";
	//
	// Mockito.when(shipRepo.findByCountryAndState(countryId, state))
	// .thenReturn(null);
	//
	...
	// product.setLength(8);
	//
	// Mockito.when(productRepo.findById(productId))
	// .thenReturn(Optional.of(product));
	//
	// float shippingCost = shipService.calculateShippingCost(productId,
	// countryId, state);
	//
