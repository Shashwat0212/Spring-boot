...
public class ShippingRateService {
	...
	public void save(ShippingRate rateInForm)
			throws ShippingRateAlreadyExistsException {
		ShippingRate rateInDB = shipRepo.findByCountryAndState(
				rateInForm.getCountry().getId(), rateInForm.getState());
		boolean foundExistingRateInNewMode = rateInForm.getId() == null
				&& rateInDB != null;
		boolean foundDifferentExistingRateInEditMode = rateInForm
				.getId() != null && rateInDB != null
				&& !rateInDB.equals(rateInForm);

		if (foundExistingRateInNewMode
				|| foundDifferentExistingRateInEditMode) {
	...
	public ShippingRate get(Integer id) throws ShippingRateNotFoundException {
		try {
			...
			throw new ShippingRateNotFoundException(
					"Could not find shipping rate with ID " + id);
		}
	}
	...
	public float calculateShippingCost(Integer productId, Integer countryId,
		...
		ShippingRate shippingRate = shipRepo.findByCountryAndState(countryId,
				state);
